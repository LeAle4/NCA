...
if __name__ == "__main__":
    ...
    # --- Hyperparameters ---
    SIZE = 64
    C = 1                      # PDE channels (scalar field)
    T = 50                     # time-steps to keep
    STEP_MUL = 10              # PDE sub-steps per kept frame
    DT = 0.001                 # PDE dt
    TRAIN_ITERS = 200          # training iterations
    ITER_N = 1                 # NCA steps per training step
    ...
    nca = NCA(
        ...
        model_size=(64,),
        perception_kernels_names=("sobel", "laplacian"),
        periodic=False,
    )
    print(f"NCA on {DEVICE}  |  params: {sum(p.numel() for p in nca.parameters()):,}")
    ...
    trainer = NCA_PDE_Trainer(
        ...
        step_mul=STEP_MUL,
        dt=DT,
        model_filename="nca_swift_hohen",
        directory="models/",
        log_dir="runs/nca_swift_hohen",
        noise_frac=0.0,
        r=-0.1,        # PDE parameter forwarded to F_swift_hohen
    )
